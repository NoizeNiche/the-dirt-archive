...
Breadth-first photo pass for The Dirt Archive. Crawls the EDB dirt indexes,
resolves catalog products to exact EDB pages, and extracts exterior product
imagery plus source metadata. Reference URLs are stored; republication rights
are not assumed.
"""
...
OUT=PUBLIC/'catalog-effects-database-photo-manifest.js'; STATUS=ROOT/'research'/'catalog-effects-database-photo-status-01.tsv'
BASE='https://www.effectsdatabase.com'; UA='The-Dirt-Archive/1.5 (Effects Database photo research)'
...
def extract_model_links(page_html):
    out=[]; seen=set(); pat=re.compile(r'<a[^>]+href=["\']([^"\']*/model/[^"\']+)["\'][^>]*>(.*?)</a>',re.I|re.S)
...
def crawl_category(category):
    ...
    for page in range(1,MAX_PAGES+1):
        ...
        if len(fresh)<50:break
        time.sleep(.1)
    return products
def parse_meta_images(page_html):
    urls=[]; pats=[r'<meta[^>]+property=["\']og:image(?::secure_url)?["\'][^>]+content=["\']([^"\']+)',r'<meta[^>]+name=["\']twitter:image["\'][^>]+content=["\']([^"\']+)',r'<img[^>]+(?:src|data-src|data-lazy-src)=["\']([^"\']+)',r'<source[^>]+srcset=["\']([^"\']+)']
    for pat in pats:
...
def candidate_score(title,builder,target_model,url):
    hay=norm(title); nt=norm(target_model); s=0
    if hay==norm(f'{builder} {target_model}'):s+=25
    if nt and nt in hay:s+=18
    for word in norm(builder).split():
        if len(word)>=4 and word in hay:s+=3
    if '/model/' in url.lower():s+=4
    if any(x in url.lower() for x in ('reissue','similar','related')):s-=2
    return s
def load_catalog():
    data=json.loads((PUBLIC/'data.json').read_text(encoding='utf-8')); pedals=list(data.get('pedals',[])); builders={str(b.get('builder_id')):b.get('name','') for b in data.get('builders',[])}
...
def main():
    ...
    for p in pedals:
        ...
        ranked=sorted(((candidate_score(title,builder,model,url),title,url) for url,title in by_url.items()),reverse=True); best=[x for x in ranked if x[0]>=12][:5]
        if not best:status.append([p.get('pedal_id',''),builder,model,0,'','UNRESOLVED']);continue
        images=[]; primary_page=best[0][2]
        for score,title,url in best[:3]:
            ...
            if len(images)>=6:break
        if images:
            manifest[model]={'src':images[0],'page':primary_page,'credit':'Effects Database reference page','license':'Reference-only; see source page for rights/attribution','source':'Effects Database','gallery':images[:6]}; status.append([p.get('pedal_id',''),builder,model,len(images),primary_page,'FOUND'])
        else:status.append([p.get('pedal_id',''),builder,model,0,primary_page,'PAGE_FOUND_NO_IMAGE'])
