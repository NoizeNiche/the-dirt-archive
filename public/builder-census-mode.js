(()=>{
  const run=()=>{
    ...
    const qualification=b=>b.qualification==='verified_dirt_maker'?'<span class="qualification-badge">DIRT-MAKER VERIFIED</span>':'<span class="qualification-badge pending">RESEARCH STATUS</span>';
    ...
    window.renderBuilderCensus=()=>{
      ...
      const verified=rows.filter(b=>b.qualification==='verified_dirt_maker').length;
      ...
      <p class="subhead">The people, companies, workshops and historical names that made dirt devices. A builder must clear the Archive's dirt-device qualification gate before being treated as a verified maker.</p>
      <div class="research-status"><span class="research-badge">BUILDER-FIRST MODE</span><span>${rows.length} builder records</span><span>${verified} dirt-makers verified</span><span>${dirt.length} currently promoted dirt records</span></div></section>
      <div class="builder-census-grid">${rows.map((b,i)=>{const n=countFor(b);return `<a class="builder-feature" href="${builderHref(b)}"><div><div class="eyebrow">${String(i+1).padStart(3,'0')}</div><div style="margin:5px 0 11px">${qualification(b)}</div><h2>${escText(b.name||'Unnamed builder')}</h2><p>${escText(b.description||b.notes||'Historical builder record. Pedal catalog pending or expanding.')}</p><div class="detail-meta"><span class="pill">${escText(b.country||b.region||'Origin unknown')}</span><span class="pill">${escText(b.status||'Research record')}</span>${b.dirt_evidence?`<span class="pill">${escText(b.dirt_evidence)}</span>`:''}</div></div><div class="statbox"><span class="n">${n}</span><span class="l">dirt records now</span></div></a>`}).join('')}</div>`;
    };
    window.home=()=>{
