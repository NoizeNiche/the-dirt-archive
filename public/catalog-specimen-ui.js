(() => {
  ...
  function addBlock(){
    ...
    const gallery=list.length?`<div class="specimen-gallery">${list.map(s=>{
      ...
      return `<figure class="specimen-item"><a href="${esc(s.src)}" target="_blank" rel="noopener"><img src="${esc(s.src)}" alt="${esc(title)} ${esc(s.role||'specimen')} photograph" loading="lazy" referrerpolicy="no-referrer"></a><figcaption class="specimen-meta"><div class="specimen-role">${esc(s.role||'Specimen')}${s.era?` · ${esc(s.era)}`:''}</div><div class="specimen-caption">${esc(s.caption||'Visual identification specimen.')}</div><div class="specimen-source"><a href="${esc(s.page||s.src)}" target="_blank" rel="noopener">Source / file page ↗</a><br>${esc(s.credit||'Source credit recorded in registry.')}</div><div class="specimen-status ${cls}">${status==='cleared'?'Rights status: cleared for Archive use':'Rights status: external reference only'}</div></figcaption></figure>`;
    }).join('')}</div>`:'';

    const strip=document.createElement('div');
    strip.className='specimen-strip';
    strip.innerHTML=`<div class="specimen-kicker">Visual specimen index</div><p>${list.length?`This page currently indexes ${list.length} documented visual specimen${list.length===1?'':'s'}. Future entries can represent factory variants, colorways, OEM versions, reissues and unusual production changes.`:'No independently registered specimen image yet. External searches provide a visual reference doorway until a source is documented and rights-reviewed.'}</p>${gallery}<div class="specimen-links"><a href="${buildUrl(title,'front')}" target="_blank" rel="noopener">Front examples ↗</a><a href="${buildUrl(title,'vintage')}" target="_blank" rel="noopener">Vintage examples ↗</a><a href="${buildUrl(title,'variant')}" target="_blank" rel="noopener">Variants ↗</a></div><div class="specimen-plate-note">Identification workflow: compare enclosure shape, graphics, controls, treadle hardware, labeling and known production variants. External photographs are identification aids, not automatically cleared Archive assets.</div>`;
