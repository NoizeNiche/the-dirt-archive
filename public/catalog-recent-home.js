(() => {
  ...
  const renderEra=()=>{
    ...
    if(!Array.isArray(window.DATA?.pedals))return false;
  ...
  const ensureEraNav=()=>{
    ...
    if(!nav||nav.querySelector('[data-era-nav]'))return;
    const a=document.createElement('a');a.href='#/era';a.dataset.eraNav='';a.textContent='Eras';
    nav.insertBefore(a,nav.querySelector('a[href="#/identify"]')||null);
  };
  ...
  const waitForData=()=>{
    ...
    const safeHome=()=>{
      ...
      if(recent.length<5){
        ...
        for(const p of fallback){if(recent.length>=5)break;recent.push(p)}
      }
  ...
  window.addEventListener('hashchange',()=>setTimeout(()=>{ensureEraNav();if(renderEra())return;const page=location.hash.replace(/^#\/?/,'').split('/').filter(Boolean)[0];if(!page&&Array.isArray(window.DATA?.pedals))window.home?.();},35));
  waitForData();
})();
