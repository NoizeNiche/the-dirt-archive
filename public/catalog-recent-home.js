(() => {
  const RECENT_HOME_MODELS=['Old Zoo Lion Meat','Wah-ocTo-Fuzz','Arabica','Limited Edition Fluff Drive','Bearded Vulture'];
  ...
  const recentHomeRecords=()=>RECENT_HOME_MODELS.map(name=>DATA.pedals.find(p=>p.model_name===name)).filter(Boolean).slice(0,5);
  const originalHome=home;

  home=function recentHome(){
    if(!Array.isArray(DATA.builders)||!Array.isArray(DATA.pedals)) return originalHome();
...
// Builder census is deliberately loaded after the base application so it can
// wait for normalized DATA without introducing more fetch-wrapper layers.
(() => {
