(() => {
  ...
  function catalogPedalForCard(card){
    const title=card.querySelector('h3')?.textContent?.trim()||'';
    const id=card.dataset.pedalId||card.getAttribute('data-pedal-id')||card.dataset.id||'';
    return {title,id};
  }
  function findEntry(manifest,ref){
    const m=manifest||{};
    if(ref.id && m[ref.id]) return m[ref.id];
    if(ref.title && m[ref.title]) return m[ref.title];
    const needle=ref.title.toLowerCase();
    for(const [k,v] of Object.entries(m)){
      if(v?.model && String(v.model).toLowerCase()===needle) return v;
    }
    return null;
  }
  function combinedEntry(ref){
    const core=findEntry(window.DIRT_PHOTO_MANIFEST,ref);
    const edb=findEntry(window.DIRT_EDB_PHOTO_MANIFEST,ref);
    const gallery=[];
    ...
    return {src:lead.src,page:lead.page||edb?.page||core?.page||'',credit:lead.credit||edb?.credit||core?.credit||'',license:lead.license||edb?.license||core?.license||'',gallery};
  }
  function render(card,title,entry){
    const gallery=(entry?.gallery||[]).filter(usable).slice(0,MAX_IMAGES);
    if(!gallery.length&&entry?.src&&usable(entry))gallery.push(entry);
    if(!gallery.length)return;
    const holder=card.querySelector('.pedal-image');
    if(holder&&!holder.querySelector('img')){
      const im=gallery[0]; holder.classList.add('photo-harvest-lead'); holder.innerHTML='';
      const a=document.createElement('a'); a.href=im.page||im.src; a.target='_blank'; a.rel='noopener noreferrer';
      const img=document.createElement('img'); img.src=im.src; img.alt=`${title} reference photograph`; img.loading='lazy'; img.referrerPolicy='no-referrer'; img.onerror=()=>a.remove();
      a.appendChild(img); holder.appendChild(a);
    ...
    wrap.innerHTML='';
    const head=document.createElement('div'); head.className='photo-harvest-head';
    head.innerHTML=`<span class="photo-harvest-label">PHOTO ARCHIVE</span><span class="photo-harvest-status">${gallery.length} exterior reference${gallery.length===1?'':'s'}</span>`; wrap.appendChild(head);
    const grid=document.createElement('div'); grid.className='photo-harvest-grid';
    gallery.forEach((im,i)=>{const a=document.createElement('a');a.href=im.page||im.src;a.target='_blank';a.rel='noopener noreferrer';a.title=`${im.title||title} · ${im.license||''}`;const img=document.createElement('img');img.src=im.src;img.alt=`${title} exterior reference photograph ${i+1}`;img.loading='lazy';img.referrerPolicy='no-referrer';img.onerror=()=>a.remove();a.appendChild(img);grid.appendChild(a)});
    wrap.appendChild(grid);
    const note=document.createElement('div'); note.className='photo-harvest-note';
    note.textContent='Exterior reference photography only. Source and rights metadata remain attached to the research record; discovery does not grant republication permission.'; wrap.appendChild(note);
  }
  async function run(){
    await loadScripts();
    window.imageFor=window.imageFor||((p)=>combinedEntry({title:p?.model_name||'',id:p?.pedal_id||''}));
    document.querySelectorAll('.pedal-card').forEach(card=>{const ref=catalogPedalForCard(card);if(ref.title)render(card,ref.title,combinedEntry(ref))});
  }
