...
The canonical tracker currently parses to **3,819 total records / 3,818 public records / 1,274 researched records / 952 pictured records / 322 public researched-photo-pending records / 2,866 public photo-missing records**. The tracker itself contains 2,867 records with Picture=NEEDED because it includes one non-public identity placeholder. The latest completed cache pass archived two additional verified local photos (Alien Rabbit Magic Drive and Analog Sound Scream For Cream OD), moving the researched-photo-pending count from 322 to 320. The recovery extractor has since been expanded to read common zoom/gallery attributes and CSS background-image/data-background sources, normalizing relative URLs before the same image-content verification. A fresh bounded cache run 936 is queued to exercise that path. Subsequent recovery leads were added for Aether Electronic Lenore, Alien Amplification Origami Overdrive, and ADV Systems #overdrive. The extractor was then hardened again to prefer lazy-loaded srcset pedal images when present. These changes will be exercised by the next cache cycle. Additional exact recovery leads were added for AMT Electronics B-1 BG-Sharp, R-1 Rectifier, SS-11B, and Alen Geere Crown Centaur. The lazy-image extractor also now recognizes data-srcset/data-lazy-srcset and full-resolution source attributes. One additional tracker record is the non-public identity placeholder used by the archive workflow.
...
Recent cache passes archived additional verified local pedal images, and the current recovery workflow continues to use curated exact source pages before broader discovery. The latest source-page maintenance added exact photo-source leads for Brantone Electronics Green Manalishi Overdrive, Brantone Electronics Woodstock - Silicon Fuzz Face, Buffalo FX CVIII - BC108 Silicon Fuzz, Buffalo FX Fuzz Face Si, Buffalo FX Tonebender Mk3, and CatastroFX Deface Mk.III. Two additional exact recovery leads were added to `research/PHOTO_SOURCE_OVERRIDES.csv`: Accel OD-SS Express Overdrive and Accel Stompzilla Fuzz. They remain photo-pending until a scheduled cache pass archives a confirmed local asset. A subsequent maintenance pass added exact recovery leads for Amsterdam Cream Big Eye Fuzz and Alien Rabbit Magic Drive; both likewise remain photo-pending until the cache archives a confirmed local asset. This pass added six more exact recovery leads: AJcustom Distortion, Alber FU-1000, Alber GA-1040 Gain, Alcove ALP-200 Overdrive, Aleatorik Operation 1, and Alen Geere Loverdrive. These are leads only and remain photo-pending until a verified local asset is actually archived. This pass also seeded ten exact model-page leads: Alen Geere Methoxy Overdrive, Analog Sound Scream For Cream OD, Analog Sound Smooth O' Drive, Analog Sound The Boogie Man, Audio Monk Fuzzbeard, Audio Monk Fuzzpotion - Bass Fuzz, Audio-Phonic R4 Fuzz, Audio-Phonic Twin-Fuzz, Aural Dream Bold Distortion, and Aural Dream Intense Distortion. These likewise remain photo-pending until the cache archives confirmed local assets. Five more exact photo-recovery leads were then seeded: Aul Instruments Fuzz, Anarchy Audio Australia Baa Bzz, Anarchy Audio Australia Deadwoods - Chainsaw Fuzz, Audile Bass TOAD, and Automat Audio Devices Triplegänger. These remain photo-pending until verified local assets are archived. AMT Electronics BS British Sound was also added from its exact Effects Database record. The live tracker was separately reconciled during this pass to 952 pictured, 322 researched-photo-pending, and 2,866 public photo-missing records, correcting stale headline counts without changing catalog identities. An exact Effects Database lead for Analog King Fuzz Machine - Germanium Fuzz + Overdrive was added as well. The controlled photo-recovery sentinel was introduced at `research/PHOTO_RECOVERY_KICK`, and the normal scheduled cache lane has since resumed. The first resumed run (933) exposed a parser-level regression in the new HTTP fallback and failed before recovery; that regex escaping defect is fixed. Run 934 then exposed a missing loop-close brace in the same fallback section; that structural defect is also fixed. A corrected bounded recovery run 935 is actively processing, and the cache workflow now includes a node syntax preflight for future recovery-script edits. Bigfoot Engineering Bigfoot Germanium EQ Fuzz also received an exact manufacturer-page recovery lead, and manufacturer-backed leads were added for Audiostorm ADNA, F258 Drive, Otherworld Overdrive, and BlackOutEffectors Blunderbuss Musket. Beta Aivin BOD-2 Bass Overdrive was also added from its exact Effects Database model record and eBay image trail.
