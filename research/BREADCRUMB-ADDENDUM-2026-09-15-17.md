...
- updated `public/catalog-research-runtime.js`

This pass validates the source trails behind the highest-priority PROMOTE lineage edges and then wires only those validated relationships into the public runtime. Twelve edges now have an explicit `VALIDATED_FOR_PUBLIC` state with preserved source provenance and notes on why identities remain separate.
...
The runtime reads a dedicated derived `public/lineage.json` dataset and adds a `Lineage & relationships` section to eligible pedal detail pages. Research-only fields and HOLD_FOR_REVIEW relationships are not exposed by this renderer.
...
1. Run a runtime smoke audit against representative linked pedals and confirm graceful no-lineage behavior.
2. Reconcile additional completed ledgers for duplicate/alias collisions before expanding the public lineage set.
