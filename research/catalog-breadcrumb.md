...
Active discovery corpus now includes `builder-census-01.tsv` through `builder-census-29.tsv`.
...
The new-creator return is **still not low enough to stop**. The Tone Bender, Big Muff, Fuzz Face, DOD 250, Fuzzrite, Buzzaround, Harmonic Percolator and Rangemaster family trees continue to expose small historical builders, revivalists, micro-builders and badge/OEM branches that disappear from broad manufacturer lists. The Buzzaround pass adds another important pattern: some small builders documented substantial dirt catalogs around a single famous family, while others may represent one-off or tiny-run copies. The Archive should preserve both without implying equal historical weight.

## Next pickup point
**Stay in Queue A.** Continue through the remaining classic-family branches, with priority on Tube Screamer-related creators and the deeper Fuzzrite, Harmonic Percolator, Buzzaround and Rangemaster nodes not yet reconciled. At the next stage, start identifying candidates that have accumulated enough independent evidence to move from `CENSUS_ONLY` to `DISCOVERED`, while separately flagging duplicate names, aliases and likely same-builder lineages. Only after that reconciliation should Queue B begin creator-by-creator complete catalog enumeration.
