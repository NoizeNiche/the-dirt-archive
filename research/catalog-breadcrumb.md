...
Active discovery corpus now includes `builder-census-01.tsv` through `builder-census-30.tsv`.
...
The new-creator return is **still not low enough to stop**. The Tone Bender, Big Muff, Fuzz Face, DOD 250, Fuzzrite, Buzzaround, Harmonic Percolator and Rangemaster family trees continue to expose small historical builders, revivalists, micro-builders and badge/OEM branches that disappear from broad manufacturer lists. The Fuzzrite pass adds another strong pattern: the same source family can produce clones, interpreted recreations, original-designer revivals and small builders with their own wider catalogs. The Archive should preserve those distinctions rather than treating all “Fuzzrite” products as one homogeneous clone class.

## Next pickup point
**Stay in Queue A.** Continue through the remaining Tube Screamer-related branches and then circle back to unresolved classic-family candidates for duplicate/alias/OEM reconciliation. Begin promotion of candidates only where independent evidence establishes a real builder identity or a clearly documented historical manufacturing/reissue node. Once repeated family sweeps produce mostly duplicates, aliases and low-evidence candidates, run the master reconciliation across `builder-census-01.tsv` through `builder-census-30.tsv`. Only after that reconciliation should Queue B begin creator-by-creator complete catalog enumeration.
