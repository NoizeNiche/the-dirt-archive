...
Active discovery corpus now includes `builder-census-01.tsv` through `builder-census-26.tsv`.
...
The new-creator return is **still not low enough to stop**. The Tone Bender and Big Muff family trees continue to expose small historical builders, badge brands and regional copies that disappear from broad manufacturer lists. The 2026 Elk/Big Muff archival work is particularly useful because it demonstrates how apparently separate brands can collapse into one manufacturing lineage only after advertising, catalog and surviving-object evidence is compared.

## Next pickup point
**Stay in Queue A.** The next highest-yield move is a Fuzz Face + DOD 250/Distortion+ related-tree sweep, followed by Tube Screamer, Fuzzrite, Buzzaround, Harmonic Percolator and Rangemaster/treble-booster family trees. Cross-check every candidate against the existing census before adding it. Once several classic-family sweeps return mostly duplicates, known OEM branches and low-evidence candidates, run the master reconciliation across `builder-census-01.tsv` through `builder-census-26.tsv`. Only after reconciliation should Queue B begin creator-by-creator complete catalog enumeration.
