...
`public/catalog-thumbnails-33.js`, `public/catalog-thumbnails-34.js`, and `public/catalog-thumbnails-35.js` are media registries. Runtime overlays attach the existing image resolver after the main app loads. New images are only eligible for public card rendering when rights status and republishing decision are explicitly cleared. Reference-only image leads remain useful research breadcrumbs but are not silently published.

Current seeded reference leads include Klinger Buzz Fuzz, KMA Wurm, Kittycaster Kittyface, KHDK No.1, Kingsley Harlot V3, Phase 34 leads for Mojo Hand, DenTone, Erafuzz, and Ghost Effects, plus Phase 35 leads for Arcane Analog, Creepy Fingers, Sabbadius, and Stone Deaf FX. Phase 35 image leads remain rights-pending. Search results supplied useful specimen imagery from Rakuten/Guitar Planet, Reverb, Chicago Music Exchange, Mass Street Music, Cool Guitar Shop, and Premier Guitar; those sources are research references unless permission/license is separately cleared.

## Immediate next queue
Continue mining historical and boutique builder indexes for uncaptured builders, prioritizing:
- remaining historical Tone Bender / Buzzaround / Tone Machine specialists
- smaller North American and European boutique builders surfaced by Effects Database indexes
- notable Japanese and Australian historical builders not yet fully enumerated
...
Phase 35 added a strong vintage-derived specialist quartet. Arcane Analog is documented with 10 products spanning Buzzaround, Fuzz Face, Tone Bender, Vox Tonebender and Big Muff-derived families, while its current catalog visibly continues that emphasis. Creepy Fingers is documented as Brad Davis's Fullerton, California operation, founded in 2008, with 23 products and an unusually rich run of fuzz families including Buzzaround, Zonk-related Hornskewer, Magna Bender, Mostratono and later updates. Sabbadius is Nicolás Sabbadin's Córdoba, Argentina company, with a 16-product Effects Database historical snapshot and current retail examples that extend the catalog. Stone Deaf FX has a 13-product Effects Database snapshot with the PDF, Fig Fumb, Kliptonite, Rise & Shine, Q Boost, Warp Drive and related families, and period documentation places the company in the UK. These remain discovery-level observations pending product-by-product verification.
