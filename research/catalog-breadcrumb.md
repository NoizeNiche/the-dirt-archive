...
Active discovery corpus now includes `builder-census-01.tsv` through `builder-census-27.tsv`.
...
The new-creator return is **still not low enough to stop**. The Tone Bender, Big Muff, Fuzz Face and DOD 250 family trees continue to expose small historical builders, revivalists, micro-builders and badge/OEM branches that disappear from broad manufacturer lists. The Fuzz Face pass also reinforced the value of primary builder histories: Dave Fox's own Crest/Foxrox account establishes a clean historical manufacturing node that directory-only research could easily blur into the later Dunlop reissues.

## Next pickup point
**Stay in Queue A.** Move next through Tube Screamer, Fuzzrite, Buzzaround, Harmonic Percolator and Rangemaster/treble-booster family trees, with particular attention to historical regional copies, small one-person builders and OEM/badge relationships. Cross-check every candidate against the existing census before adding them. Once several additional classic-family sweeps return mostly duplicates, known OEM branches and low-evidence candidates, run the master reconciliation across `builder-census-01.tsv` through `builder-census-27.tsv`. Only after reconciliation should Queue B begin creator-by-creator complete catalog enumeration.
