...
- Big Muff specimen work: Triangle, Ram’s Head, Red/Black, Op-Amp, Tone Bypass, later transistor and Russian/Sovtek branches.
...
## Next queue
**Current priority is builder discovery.** Continue expanding the builder census using independent historical, current-market, specialist and regional source pools. Convert promising candidates into canonical builder records only after entity type and manufacturer attribution are verified. Continue country-by-country sweeps outside the core US/UK/European pool, with special attention to Latin America, Southeast Asia, India, Africa, China and other regions where local builders can be hidden from mainstream pedal directories. Treat large manufacturers, boutique builders, OEM factories, private labels, clone-focused operations and designer lineages as distinct entity types. Once new geographic/source sweeps begin yielding mostly duplicates, stop expanding blindly and perform a dedicated census reconciliation pass before moving to builder-by-builder catalog exhaustion.
