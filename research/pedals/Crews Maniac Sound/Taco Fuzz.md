# Crews Maniac Sound — Taco Fuzz

## PRP identity
- **Archive parent:** Taco Fuzz
- **Builder:** Crews Maniac Sound
- **Catalog type:** Fuzz
- **Identity:** Crews Maniac Sound's octave-oriented fuzz.

## What this pedal is
Taco Fuzz is an octave fuzz from Crews Maniac Sound. The builder's April 2010 material describes it as producing a strong upper-octave effect with a sustained, bending response, controlled by Gain, Tone, and Volume. [1][2]

## Colorways
- Documented examples use the period Crews enclosure and Taco graphic treatment.
- Cosmetic differences are treated as colorways unless a circuit change is documented.

## Versions and factory options
### Taco Fuzz production
- Gain
- Tone
- Volume
- Upper-octave fuzz character
- Standard external pedal operation

The original production was later discontinued. Crews' own interview explains that a specific capacitor used in the pedal became difficult to source consistently, making continued production impractical. [3]

## Version changes
No reliable numbered V1/V2 production sequence was verified. The archive keeps Taco Fuzz as one parent identity and records the production-end history as a lifecycle note rather than a separate version.

## Transistor
- **Technology:** Exact transistor/device information is not publicly established in the reliable builder material checked.
- **Exact transistor/device:** Unknown.

## Diode
- **Type:** Exact production clipping/rectification device was not publicly established.
- **Exact part:** Unknown.

## Sound
Taco Fuzz is built around a strong upper-octave voice that becomes especially pronounced when the player uses sustained notes and bends. The Tone and Gain controls let the effect move from a more cutting octave-fuzz texture toward thicker, dirtier and more sustained fuzz. [1][2]

## Sources checked
1. Crews Maniac Sound — Taco Fuzz announcement: https://crewsmaniacsound.blogspot.com/2010/04/taco-fuzz.html
2. Crews Maniac Sound — Taco Fuzz demo/video reference: https://www.youtube.com/
3. Effects Database — Crews Maniac Sound interview/history: https://www.effectsdatabase.com/interviews/brands/crews

## Photo
- **Archive status:** No local photo archived in this research pass.
- **Exact-model visual references checked:** Crews Taco Fuzz product material and exact-name video references.
