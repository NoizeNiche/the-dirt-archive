...
Detail links also carry the active dirt-type filter into the detail URL. Builder and version links on the detail page preserve that type context, so a visitor can move sideways through the archive without unexpectedly broadening the results. Builder counts on the detail sidebar use that same context when a dirt type is active.
